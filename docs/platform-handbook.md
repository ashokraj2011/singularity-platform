...
Agent Runtime treats that contract pin as mandatory for production-class template activation. When an `AgentTemplate` update leaves the template `ACTIVE`, Agent Runtime waits for Prompt Composer to mint and record the `contractId`/`contractHash`. `AGENT_CONTRACT_MINT_TIMEOUT_SEC` defaults to `15` and is bounded `1..300` for the Prompt Composer mint request. In local development a mint outage is still logged and tolerated; in production-class environments, or when `AGENT_CONTRACT_MINT_REQUIRED=true`, failed minting reverts the template status and returns `CONTRACT_MINT_REQUIRED` so an agent cannot silently publish without replay evidence.
