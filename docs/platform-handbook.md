...
Legacy `/execute` MCP invoke timeouts are bounded too. `CONTEXT_FABRIC_MCP_INVOKE_TIMEOUT_SEC` defaults to `480`, malformed or below-1 values fall back to the default, and request-level `limits.timeoutSec` / `timeout_sec` values clamp at `7200`. `DEEP_REASONING_BUDGET_TOKENS` defaults to `0` and clamps at `32768`, so an invalid Anthropic thinking-budget setting disables the feature instead of failing a run.
