...
Run `./singularity.sh urls` to print the current local endpoint map, and `./singularity.sh topology` to count and validate the active containers. The machine-readable topology source is `docs/platform-topology.json`; the default Docker core currently runs 9 long-running containers: one unified frontend (`platform-web`), one consolidated agent/tools backend (`platform-core`), four separate product backends (`iam-service`, `workgraph-api`, `context-api`, `code-foundry-api`), and three storage containers (`at-postgres`, `wg-postgres`, `wg-minio`). Two idempotent Postgres bootstrap one-shots (`at-postgres-bootstrap`, `wg-postgres-bootstrap`) must complete successfully; the Workgraph bootstrap provisions the non-bypass runtime DB role. LLM Gateway, MCP, verifier, compressor, audit-governance, and legacy split UIs are optional local profiles or remote services.
...
|---|---:|---|
| `at-postgres` | `5432` | Agent runtime, tool service, prompt composer, Context Fabric, Code Foundry, and IAM (`singularity_iam`) data. |
...
| `singularity-code-foundry` | Code Foundry API; UI routes are served by Platform Web. |
...
This starts the core Docker Compose stack. Local LLM Gateway, MCP, verification, compression, and audit-governance are optional profiles or remote services. Code Foundry API is included in core because `/foundry` is part of Platform Web.
