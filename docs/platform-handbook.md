...
Their default and maximum request timeouts are centrally configured through
`MCP_COMMAND_TOOL_DEFAULT_TIMEOUT_MS`, default `120000`, and
`MCP_COMMAND_TOOL_MAX_TIMEOUT_MS`, default `600000`; both are bounded
...
Platform Web's Workgraph service-token auto-mint path bounds IAM bootstrap login
and service-token mint calls with `WORKGRAPH_PROXY_TOKEN_MINT_TIMEOUT_SEC`,
default `10`, bounded `1..300`. This path is development/local convenience only;
...
### 14.2.1 Workflow Authorization Spine

Workflow access is enforced at the WorkGraph API boundary with a typed resource/action decision. A request must pass the tenant boundary first, then explicit workflow grants, team/role access, owning capability permissions, and workflow visibility. An explicit deny wins over an allow. Gallery visibility never grants edit or start access.

Template list/detail/create/import/duplicate/publish/archive/delete, instance reads and exports, lifecycle simulation/checkpoints/replay, WorkItem routing/claim/start/edit/archive, approvals, runner queues, and workflow operations are all protected. Operations reads require `workflow:operations:view`; replay, delivery retry, and runner requeue use separate permissions. Raw payloads, delivery URLs, documents, credentials, and code-change content are redacted unless the caller also has `workflow:audit:view`.

At run start, WorkGraph stores an immutable `WorkflowAuthorizationSnapshot` containing actor, tenant, workflow, capability, effective roles/permissions, grants, policy version, and digest. Live checks remain mandatory for approvals, secrets, MCP/tool calls, Git side effects, runner claims, replay/resume, artifact publication, and takeover. Inspect it with `GET /api/workflow-instances/:id/authorization`.

IAM authorization requests require `tenant_id`. Human JWTs can check only their own subject; service tokens need the explicit `authz:check` scope and a matching `tenant_ids` allowlist. The unified access view is `/identity/effective-access` and `GET /api/authz/effective-access?tenant_id=<tenant>`.

### 14.3 Tenant Isolation

Non-strict mode is still authenticated and capability-scoped. In strict mode, Workgraph `AGENT_TASK` dispatch must resolve `tenantId`/`tenant_id` from node config, workflow context, vars/globals, or WorkItem input before it can call Context Fabric; Context Fabric then also rejects calls without `run_context.tenant_id` when `REQUIRE_TENANT_ID=true`. Workflow instances and browser run snapshots persist first-class `tenantId` values, and strict-mode workflow-instance routes require `X-Tenant-Id` or `tenant_id` to match before reads, mutations, SSE/event access, or pending-execution claim/complete. Strict mode also guards Workgraph run-adjacent AgentRun, ToolRun, approval, consumable, document, code-change, receipt, insight, evidence-pack, runtime inbox, Workbench definitions, internal artifact-fetch, and feature-flag surfaces. IAM service-token minting carries `tenant_ids`, Workgraph/Context Fabric strict-mode service-token resolvers reject broad or mismatched tokens, and deploy preflight requires `IAM_SERVICE_TOKEN_TENANT_IDS` plus `WORKGRAPH_INTERNAL_TOKEN_TENANT_IDS`; `./singularity.sh config production-guardrails --tenant-id <tenant>` writes both allowlists. `bin/check-workgraph-db-tenant-isolation.py` now verifies live Workgraph tenant data and requires forced RLS by default for production-class deploys. Workgraph has request-scoped tenant DB context helpers, a Prisma transaction proxy, workflow-instance spine, run authorization snapshots, AgentRun, ToolRun, approval, consumable, document, code-change, receipt, insight/evidence, runtime inbox, Workbench definitions, and internal artifact-fetch routes using tenant-scoped DB transactions, checked-in RLS policy scaffolding, a runtime-spine backfill tool, and a guarded `bin/enable-workgraph-forced-rls.py` cutover. Workflow templates and access grants now carry tenant ownership and are protected by the central typed resource/action decision layer; their forced-RLS cutover is intentionally staged until the remaining legacy direct-Prisma designer/runtime paths are transaction-scoped. Strict production targets still need forced RLS enabled for all tenant-sensitive tables, or a schema-per-tenant deployment model, before they should be described as complete hard isolation.
