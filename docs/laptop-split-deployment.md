...
- **Portal menu bounces back to the portal** — the nav links default to edge-gateway single-origin paths (`/workflow`, `/workbench`, …), which are correct at the gateway `:8085` but resolve to the portal itself on its standalone port (`:5180`). The portal reads link targets at **runtime** from `/env.js` (`PORTAL_LINK_*` env on the portal container, written at container start — no rebuild); the direct overlay points them at the per-app ports. So either **enter via the edge-gateway `http://localhost:8085`**, or rebuild + recreate the portal so it picks up the `PORTAL_LINK_*` overrides for `:5180`.
