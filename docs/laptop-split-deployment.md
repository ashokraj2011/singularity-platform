...
**Run it:** create a `feature` work item (routes to this SDLC at priority 300), set `story` + `repoUrl`, run it with the laptop mcp + gateway up and `copilot` on PATH. Approve each phase; the run gates at **Verify documents** before the push.
