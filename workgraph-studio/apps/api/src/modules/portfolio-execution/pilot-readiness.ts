...
export function evaluatePilotReadiness(projectId: string, evidence: PilotEvidence) {
  ...
  const checks: PilotReadinessCheck[] = [
    ...
    { key: 'capability-brief', label: 'Assigned capability impact brief is assessed', ok: evidence.capabilityLinks === 1 && evidence.assessedCapabilityLinks === 1, fixRoute: synthesis('hub'), evidence: `${evidence.assessedCapabilityLinks}/${evidence.capabilityLinks} assigned capability assessed` },
