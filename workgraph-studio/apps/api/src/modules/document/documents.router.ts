...
documentsRouter.get('/', async (req, res, next) => {
  try {
    ...
    const tenantId = requireTenantFromRequest(req, 'document listing')
    const docs = await withTenantDbTransaction(prisma, async () => {
      if (tenantIsolationStrict() && instanceId) await assertWorkflowInstanceTenant(req, instanceId)
...
documentsRouter.get('/:id', async (req, res, next) => {
  try {
    const doc = await withTenantDbTransaction(prisma, async () => {
      await assertDocumentTenant(req, req.params.id)
      return prisma.document.findUnique({ where: { id: req.params.id } })
    })
...
documentsRouter.post('/upload', upload.single('file'), async (req, res, next) => {
  try {
    ...
    if (tenantIsolationStrict() && !instanceId) {
      throw new ForbiddenError('TENANT_ISOLATION_MODE=strict requires instanceId when uploading a document')
    }
    ...
    const doc = await withTenantDbTransaction(prisma, async () => {
      ...
      await logEvent('DocumentUploaded', 'Document', created.id, req.user!.userId, {
        taskId: taskId ?? null,
        nodeId: nodeId ?? null,
...
documentsRouter.post('/links', async (req, res, next) => {
  try {
    ...
    if (tenantIsolationStrict() && !instanceId) {
      throw new ForbiddenError('TENANT_ISOLATION_MODE=strict requires instanceId when attaching a document link')
    }
    ...
    const doc = await withTenantDbTransaction(prisma, async () => {
      ...
      await logEvent('DocumentLinkAttached', 'Document', created.id, req.user!.userId, {
...
documentsRouter.delete('/:id', async (req, res, next) => {
  try {
    const doc = await withTenantDbTransaction(prisma, async () => {
      ...
      if (!found) throw new NotFoundError('Document', req.params.id)
      await assertDocumentTenant(req, req.params.id)
