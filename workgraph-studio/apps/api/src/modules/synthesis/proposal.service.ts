...
export async function listProposals(workspaceId: string) {
  return { items: await prisma.studioProposal.findMany({ where: { workspaceId, tenantId: tenantId(), contractVersion: 2 }, orderBy: { createdAt: 'desc' }, include: { items: { orderBy: { ordinal: 'asc' } } } }) }
}
export async function getProposal(proposalId: string) {
  const p = await prisma.studioProposal.findFirst({ where: { id: proposalId, tenantId: tenantId(), contractVersion: 2 }, include: { items: { orderBy: { ordinal: 'asc' } } } })
 ...
 * continues (so accept-selected works). ACCEPT/EDIT run the apply-registry; the proposal
 * settles from its items afterward.
 */
export async function decideProposalItems(proposalId: string, decisions: ItemDecisionInput[], actor: string) {
  const proposal = await prisma.studioProposal.findFirst({ where: { id: proposalId, tenantId: tenantId(), contractVersion: 2 }, include: { items: true } })
  ...
  for (const d of decisions) {
    ...
    if (d.decision === 'REJECT') {
      await prisma.proposalItem.update({ where: { id: item.id }, data: { status: 'REJECTED', decidedById: actor } })
      continue
    }
    if (isItemStale(item.baseContentHash, d.currentContentHash)) {
      await prisma.proposalItem.update({ where: { id: item.id }, data: { status: 'STALE', decidedById: actor } })
      continue
    }
  ...
  const fresh = await prisma.proposalItem.findMany({ where: { proposalId }, select: { status: true } })
  const settled = settleProposalStatus(fresh.map((i) => i.status as ItemStatus))
  return prisma.studioProposal.update({
    where: { id: proposalId },
