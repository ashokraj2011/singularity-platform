...
describe('Workgraph user-facing service-token boundary', () => {
  ...
  it('requires caller authorization before Platform Web injects privileged service tokens', () => {
    ...
    expect(codegen.indexOf('requireVerifiedCallerBearer(req, "Code Foundry")')).toBeLessThan(codegen.indexOf('headers.set("authorization", `Bearer ${token}`)'))
    ...
    expect(llmSettings.indexOf('requireVerifiedCallerBearer(request, "LLM settings")')).toBeLessThan(llmSettings.indexOf('mcpGet("/llm/providers")'))
