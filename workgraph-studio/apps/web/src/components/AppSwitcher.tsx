...
// M100 P3 — under the single-origin edge gateway every UI shares one origin, so
// cross-app nav uses same-origin PATH PREFIXES (not localhost:<port>). The
// VITE_LINK_* overrides remain for split-origin deployments.
function useAppLinks(): AppLink[] {
  return useMemo(() => [
    { id: 'operations', label: 'Operations', href: import.meta.env.VITE_LINK_OPERATIONS_PORTAL ?? '/operations', description: 'Health, setup, audit, readiness', icon: ServerCog },
