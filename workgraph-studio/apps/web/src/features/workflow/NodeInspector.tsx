...
const NODE_META: Record<string, {
  ...
  DIRECT_LLM_TASK: {
    ...
    description: 'Calls an LLM directly from WorkGraph while still letting an agent profile and its URL/document skills assemble the verifier prompt through Prompt Composer. The node can inject fields to fill, validate the JSON output, pause for human review, then let decision/event nodes route the result.',
    standardFields: [
      { key: 'modelAlias',    label: 'WorkGraph LLM connection alias', placeholder: 'Recommended: alias from LLM routing, e.g. anthropic-prod' },
      { key: 'provider',      label: 'Provider (ad-hoc fallback)',         placeholder: 'Used only when no alias is configured: mock | openai_compatible | openai | anthropic' },
      { key: 'baseUrl',       label: 'Base URL (ad-hoc fallback)',         placeholder: 'Used only when no alias is configured' },
      ...
      { key: 'capabilityId', label: 'Capability grounding', placeholder: 'capability uuid for URL/document knowledge retrieval' },
      { key: 'composeWithPromptComposer', label: 'Use agent prompt + skills', placeholder: 'true when agentTemplateId is set' },
      { key: 'assignmentMode', label: 'Review assignment mode', placeholder: 'DIRECT_USER | TEAM_QUEUE | ROLE_BASED | SKILL_BASED' },
      { key: 'assignedToId', label: 'Reviewer user id', placeholder: 'optional direct reviewer' },
      { key: 'teamId', label: 'Reviewer team id', placeholder: 'optional team queue' },
      { key: 'roleKey', label: 'Reviewer role', placeholder: 'owner | reviewer | qa' },
      { key: 'skillKey', label: 'Reviewer skill', placeholder: 'security-review | domain-owner' },
      ...
      { key: 'outputFields', label: 'Fields to fill (JSON)', placeholder: '{"verdict":{"type":"string","enum":["APPROVE","REJECT","SEND_BACK"],"description":"final decision"},"confidence":"number","findings":{"type":"array","items":{"type":"string"}}}', multiline: true },
  ...
  VERIFIER: {
    ...
    description: 'Runs the built-in verifier on documents produced by prior stages through the governed Context Fabric path. For a verifier agent whose prompt comes from a URL/profile and calls an LLM directly, use Direct LLM Task with outputFields + reviewRequired.',
    standardFields: [
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {/* WORKBENCH — first-class WORKBENCH_TASK editor. The stage-graph
                canvas now fully covers stages, edges, per-stage config, artifacts,
                questions, and workflow-wide settings (⚙ Settings), so the legacy
                accordion (WorkbenchStageBuilder) has been retired. */}
            {tab === 'Workbench' && (
