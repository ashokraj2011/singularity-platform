...
const ACCESS_KEYS: AccessKeyConfig[] = [
  ...
  },
  {
    ...
    description: "Bearer credential used by platform services to call the MCP runtime when it is enabled.",
    group: "runtime",
    owner: "MCP Runtime",
    kind: "bearer_token",
    envKeys: ["MCP_BEARER_TOKEN", "MCP_DEMO_BEARER_TOKEN"],
    scope: "Context Fabric / tools -> MCP",
    usedBy: ["MCP runtime", "Tool execution", "Context Fabric dial-in", "Laptop bridge"],
    rotation: "Use a long random token shared only with the MCP runtime and platform callers.",
    required: false,
  ...
  },
  {
    ...
    rotation: "Store in the LLM gateway or laptop bridge secret file, not in browser-visible config.",
    required: false,
    remoteCapable: true,
    visibleToPlatformWeb: false,
    minLength: 16,
