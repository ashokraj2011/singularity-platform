...
function GenerationWorkspace({ projectId }: { projectId: string }) {
  ...
  const compile = async () => {
    ...
    try {
      const result = await workgraphFetch<{ version: SynSpecificationVersion; warnings?: Array<{ reason: string }>; sponsorGate?: { required: boolean } }>(`/studio/projects/${projectId}/compile`, { method: "POST", body: JSON.stringify({ waiverReasons: waivers }) });
      setMessage(`Compiled specification v${result.version.version} and opened technical review${result.sponsorGate?.required ? " plus sponsor readout review" : " on the DRI fast lane"}${result.warnings?.length ? ` with ${result.warnings.length} belief warning(s)` : ""}. Generation unlocks after the required approvals.`);
  ...
  return <div className="space-y-6">
    <StageHeader eyebrow="Approved intent → Immutable contract → WorkItems" title="Compile and generate" description="Compile claims, objectives, requirements, and accepted decisions into one immutable candidate. Technical review and the threshold-based sponsor lane must consent before generation." icon={WandSparkles} />
    ...
    <SynCard className="p-5">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2"><LockKeyhole size={16} className="text-secondary" /><h2 className="font-black text-on-surface">Compile and request consent</h2></div><p className="mt-1 max-w-3xl text-sm text-on-surface-variant">Compilation checks structure, belief health, and business-objective coverage, then stores a canonical content hash. The contract becomes approved only after the required independent review lanes finish.</p><Link href={`/synthesis/business?project=${encodeURIComponent(projectId)}`} className="mt-2 inline-block text-xs font-bold text-secondary">Open objectives and sponsor readouts</Link></div><SynButton icon={LockKeyhole} disabled={busy || requirements.length === 0} onClick={() => void compile()}>{busy ? "Compiling…" : "Compile & request review"}</SynButton></div>
