...
function BusinessAlignment({ projectId }: { projectId: string }) {
  ...
  // BUSINESS_METRIC catalog. Absent or empty is fine -- the composer falls back
  // to free text, which is what every objective used before the catalog existed.
  const metricsQ = useSyn<{ items: Array<Record<string, unknown>> }>("/metadata-definitions?kind=BUSINESS_METRIC&status=ACTIVE");
  ...
  return <div className="space-y-8">
    ...
    <Section title="Objectives and bidirectional coverage" description="An objective with no work is unfunded intent. Work with no objective is unexplained scope." icon={Target}>
      <ObjectiveComposer projectId={projectId} metrics={(metricsQ.data?.items ?? []).map(item => ({ key: String(item.key ?? ""), label: String(item.label ?? item.key ?? ""), unit: String(((item.ui ?? {}) as Record<string, unknown>).unit ?? "") })).filter(item => item.key)} ownerId={projectQ.data.sponsorId ?? projectQ.data.productOwnerId ?? ""} users={(usersQ.data?.items ?? []).map(item => ({ id: String(item.id ?? item.userId ?? ""), label: String(item.name ?? item.displayName ?? item.email ?? item.id ?? "") })).filter(item => item.id)} busy={busy} onCreate={(payload) => act("objective", () => workgraphFetch(`/studio/business-alignment/objectives`, { method: "POST", body: JSON.stringify(payload) }))} />
