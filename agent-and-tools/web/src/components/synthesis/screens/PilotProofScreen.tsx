...
function PilotProof({ projectId }: { projectId: string }) {
  ...
  return <div className="space-y-7">
    ...
    <section><div className="mb-3 flex items-center gap-2"><ClipboardCheck size={17} className="text-secondary" /><div><h2 className="font-black text-on-surface">Proof obligations</h2><p className="text-xs text-on-surface-variant">Fix links preserve the selected initiative.</p></div></div><div className="overflow-hidden rounded-md border border-outline-variant bg-surface">{data.checks.map(check => <div key={check.key} className="flex items-center gap-3 border-b border-outline-variant px-4 py-3 last:border-b-0">{check.ok ? <CheckCircle2 size={18} className="shrink-0 text-secondary" /> : <CircleDashed size={18} className="shrink-0 text-tertiary" />}<span className="min-w-0 flex-1 text-sm font-semibold text-on-surface">{check.label}</span><SynChip tone={check.ok ? "success" : "tertiary"}>{check.ok ? "Evidenced" : "Pending"}</SynChip>{!check.ok ? <Link className="btn-secondary text-xs" href={check.fixRoute}>Resolve</Link> : null}</div>)}</div></section>
