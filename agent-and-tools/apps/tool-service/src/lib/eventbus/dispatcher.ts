 ...
 * Dedicated pg.Client LISTENs on `event_outbox_tool_service`, drains on
...
async function findMatchingSubscriptions(eventName: string): Promise<SubRow[]> {
  const { rows } = await pool.query<SubRow>(
    `SELECT id, target_url, secret, event_pattern
     FROM tool.event_subscriptions WHERE is_active = true`,
  );
...
async function deliverOne(
  ...
  const cur = await pool.query<{ attempts: number }>(
    `SELECT attempts FROM tool.event_deliveries WHERE outbox_id = $1 AND subscription_id = $2`,
...
async function processOutboxRow(outboxId: string): Promise<void> {
  const { rows } = await pool.query(
    `SELECT id, event_name, envelope, status FROM tool.event_outbox WHERE id = $1`,
    [outboxId],
  );
  ...
  for (const s of subs) {
    await pool.query(
      `INSERT INTO tool.event_deliveries (outbox_id, subscription_id, status)
       VALUES ($1, $2, 'queued')
  ...
  for (const s of subs) {
    const cur = await pool.query<{ status: string }>(
      `SELECT status FROM tool.event_deliveries WHERE outbox_id = $1 AND subscription_id = $2`,
      [row.id, s.id],
    );
  ...
  const remaining = await pool.query<{ c: string }>(
    `SELECT COUNT(*)::text AS c FROM tool.event_deliveries WHERE outbox_id = $1 AND status = 'queued'`,
    [row.id],
  );
  if (Number(remaining.rows[0].c) === 0) {
    await pool.query(
      `UPDATE tool.event_outbox
...
async function sweep(): Promise<void> {
  ...
  try {
    const { rows } = await pool.query<{ id: string }>(
      `SELECT id FROM tool.event_outbox WHERE status = 'pending'
    ...
    for (const r of rows) {
      try { await processOutboxRow(r.id); }
        ...
        await pool.query(
          `UPDATE tool.event_outbox SET last_error = $2, last_attempt_at = now() WHERE id = $1`,
