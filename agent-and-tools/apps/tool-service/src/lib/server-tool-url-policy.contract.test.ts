...
assert.equal(
  serverToolUrlPolicy("http://tool-service:3002/api/v1/internal-tools/search_knowledge").allowed,
  true,
  "seeded internal tool endpoints should be allowed by default",
);

assert.equal(
  serverToolUrlPolicy("http://tool-service:3002/api/v1/connector-tools/send_email").allowed,
  true,
  "seeded connector tool endpoints should be allowed by default",
);

assert.equal(
  serverToolUrlPolicy("http://tool-service:3002/api/v1/admin").allowed,
  false,
  "matching origin alone must not allow unrelated paths",
);

assert.equal(
  serverToolUrlPolicy("http://tool-service:3002.evil.test/api/v1/internal-tools/search").allowed,
  false,
...
assert.equal(
  serverToolUrlPolicy("http://127.0.0.1:3002/api/v1/internal-tools/search_knowledge").allowed,
  true,
  "baked internal localhost tool-service endpoints should still work for bare-metal/dev",
);

assert.equal(
  serverToolUrlPolicy("http://127.0.0.1:3002/admin", "http://127.0.0.1:3002/admin").allowed,
  false,
...
assert.equal(
  serverToolUrlPolicy("http://host.docker.internal:3002/api/v1/internal-tools/search_knowledge", "http://host.docker.internal:3002/api/v1/internal-tools").allowed,
  false,
