...
export const promptController = {
  ...
  async attachLayer(req: Request, res: Response) {
    const link = await promptService.attachLayer(req.params.profileId, req.body.promptLayerId, req.body.priority, req.body.isEnabled);
  ...
  async getAssembly(req: Request, res: Response) {
    const assembly = await import("../../config/prisma").then(m => m.prisma.promptAssembly.findUnique({
      where: { id: req.params.id },
      include: { layers: { orderBy: { priority: "asc" } } },
    }));
    if (!assembly) {
      res.status(404).json({ success: false, data: null, error: { code: "NOT_FOUND", message: "Assembly not found" }, requestId: res.locals.requestId });
      return;
    }
