...
async function main() {
  ...
  await rejects(/private, local, or metadata/, () => assertEventTargetUrlAllowed("http://localhost:3001/webhook"));
  await rejects(/private, local, or metadata/, () => assertEventTargetUrlAllowed("http://127.0.0.1:3001/webhook"));
  ...
  console.log("agent-service event target URL policy contracts passed");
}
