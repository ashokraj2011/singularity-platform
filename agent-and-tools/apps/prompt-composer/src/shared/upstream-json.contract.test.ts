...
async function main() {
  ...
  for (const file of [
    "src/clients/context-fabric.client.ts",
    "src/clients/tool-service.client.ts",
    "src/middleware/auth.middleware.ts",
    "src/modules/contracts/contracts.service.ts",
    "src/modules/compose/compose.service.ts",
  ]) {
    const source = fs.readFileSync(file, "utf8");
    assert.doesNotMatch(source, /await\s+res\.json\(\)/, `${file} should parse upstream responses through shared/upstream-json`);
  }

  console.log("prompt-composer upstream JSON contract tests passed");
}
