...
const developerCodeMutationPolicy = [
  ...
  "- This stage OWNS unit tests: add or extend unit tests covering the new/changed behavior and its edge cases, and run them (they must pass) before finishing. Never rely on existing tests alone for new behavior — that is incomplete work, not QA's job.",
...
const roleContracts: Array<{
  ...
  },
  {
    ...
    content: "You are a Developer Agent. Implement changes safely, prefer small reversible edits, use local AST/code tools before full-file reads. You own unit tests for the behavior you implement: add or extend tests covering new/changed behavior and edge cases, run them, and confirm they pass — code-change evidence MUST include new/updated tests with a passing run, not just 'test guidance'.",
  },
  {
    role: "QA",
...
const loopArchitectPlanTask = [
  ...
  "",
  "Risk & question discipline (avoid low-signal noise):",
  "- risk_level reflects genuine implementation hazards (breaking-change surface, a hard external dependency, an ambiguous design tradeoff) — NOT how much of the repo you have yet to read.",
  "- A fact you can resolve by reading the repo in EXPLORE is NOT a risk and NOT an open_question. \"Enum/service/test location or exact name not yet confirmed\", \"existing patterns not yet examined\", \"test-file location unverified\" are EXPLORE work — put those targets in symbols_to_inspect / target_files, never in risk_level or open_questions.",
  "- open_questions is only for what a human must decide and code-reading cannot answer (product/scope/design intent). If EXPLORE could settle it, it is not an open question.",
  "",
...
const loopArchitectExploreTask = [
  ...
  "- gaps: optional unknowns that genuinely need human or Developer attention.",
  "",
  "Resolve, don't defer: EXPLORE has full read tools — USE them to settle repo-discoverable unknowns (find_symbol/search_code/get_ast_slice for the enum/service/method; locate the test files and existing patterns). Only put in gaps what reading the code genuinely cannot resolve (a true design ambiguity or a decision that needs a human). Do NOT list \"name/location unknown\" or \"patterns not examined\" as gaps — go find them and record the answer in implementation_findings.",
  "",
...
const loopDeveloperActTask = [
  ...
  "",
  "THIS STAGE OWNS UNIT TESTS. As part of your edits this phase, add or extend unit tests covering the new/changed behavior and its edge cases. Test files you create or modify go in edits[] (they are real edits) — do NOT defer test-writing to QA and do NOT leave new tests in skipped_targets. QA verifies and reports; the Developer writes the tests. Implementing production code without accompanying tests for new behavior is incomplete work.",
  "",
  "Required output fields:",
  ...
  "",
  "Example of the correct shape when you implemented Operator.java + RuleEngineService.java AND added unit tests for the new behavior:",
  "  edits: [",
  ...
  "    {file: \"src/test/java/org/example/rules/RuleEngineServiceTest.java\", edit_type: \"apply_patch\", reason: \"added unit tests covering the new operator incl. case-insensitive + edge cases\"}",
  "  ],",
  "  skipped_targets: []",
  "Tests for new/changed behavior belong in edits[] — putting them in skipped_targets as \"QA's scope\" is wrong; this stage owns them.",
  "",
...
const loopDeveloperVerifyTask = [
  ...
  "Stage: {{stageLabel}} ({{stageKey}}) — role {{agentRole}}",
  "",
  "Edits landed in ACT (including the unit tests you added for the new behavior). Now prove they work.",
  "",
  "Run the unit tests you added/updated in ACT — they MUST be part of the run and MUST pass. A passing run that exercises only pre-existing tests does NOT satisfy VERIFY for new behavior; if your new tests aren't covered, go back to ACT (next_phase: REPAIR) and add them.",
  "",
