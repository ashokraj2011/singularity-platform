...
def _filter_phase_tools(
    ...
    Filter rules (composed in this order):
      1. If repo_access is explicitly False, strip all repo-touching
         categories (read/mutate/run/finalize). Only verify_meta +
         analyzer + the meta-tool survive.
      2. If tool_policy is set, intersect the remaining allowed list
         with the categories that policy permits.

    When neither is set, the input is returned unchanged.
    """
    if repo_access is None and tool_policy is None:
        return phase
    # Belt: repo_access=False is the most restrictive — it wins.
    if repo_access is False:
        ...
        non_repo = [
            t for t in phase.allowed_tools
            if TOOL_CATEGORY.get(t, "unknown") in ("verify_meta", "analyzer")
        ]
        return replace(phase, allowed_tools=non_repo)
    ...
    if tool_policy is not None:
        ...
        if cats is not None:  # known policy
            filtered = [t for t in phase.allowed_tools if tool_passes_policy(t, tool_policy)]
            return replace(phase, allowed_tools=filtered)
    return phase
...
def apply_execution_policy(
    ...
    if override is None:
        return base
    if override.tool_policy is None and override.repo_access is None:
    ...
    log.info(
        "stage_exec_policy: applied override stage=%s tool_policy=%s repo_access=%s",
        base.stage_key, override.tool_policy, override.repo_access,
    )
