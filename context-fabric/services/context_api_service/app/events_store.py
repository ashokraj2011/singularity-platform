...
def init_db() -> None:
    ...
    with db_conn(DB_TARGET) as conn:
        inserted_default = "now()" if conn.is_postgres else "(strftime('%Y-%m-%dT%H:%M:%fZ','now'))"
        conn.execute(
            f"""
            CREATE TABLE IF NOT EXISTS events (
                ...
                inserted_at TEXT NOT NULL DEFAULT {inserted_default}
            );
            """
        )
        try:
            suffix = "IF NOT EXISTS tenant_id TEXT" if conn.is_postgres else "tenant_id TEXT"
            conn.execute(f"ALTER TABLE events ADD COLUMN {suffix}")
        except Exception:
            pass
    ...
    with db_conn(DB_TARGET) as conn:
        for ev in events:
            ...
            try:
                ...
                if conn.is_postgres and getattr(cur, "rowcount", 1) == 0:
                    continue
                written += 1
            except Exception:
