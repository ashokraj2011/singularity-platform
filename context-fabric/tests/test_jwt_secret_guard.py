...
The runtime bridge verifies device/runtime JWTs with a shared HS256 JWT_SECRET
(laptop_bridge.py). config.py refuses to start a production-class Context Fabric
when JWT_SECRET is unset, too short, or the in-source dev default. These tests
lock that in so a future config refactor can't silently drop JWT_SECRET from the
guarded set and re-open runtime-token forgery.

Run (the repo conftest pulls sibling-service packages needing the full install):
    cd context-fabric && PYTHONPATH=services:shared \
        python3 -m pytest tests/test_jwt_secret_guard.py -q --noconftest
"""
...
from context_api_service.app import config
...
def test_config_import_fails_fast_in_prod_with_default(monkeypatch):
    ...
    JWT_SECRET must fail at import time (the module-level guard call), not just
    when the helper is invoked directly."""
    _clear_env_signals(monkeypatch)
    monkeypatch.setenv("ENVIRONMENT", "production")
