...
#   db_port     : '5432'
#
# Skips on purpose: real IAM, llm-gateway, context-memory, metrics-ledger,
# MinIO, portal, user-and-capability. The demo path (login → Agent Studio →
# run workflow → insights → audit) only needs the services this script boots.
...
EOF
  ...
  if command -v pip >/dev/null 2>&1; then
    ...
    python3 -c "import fastapi, uvicorn, httpx" 2>/dev/null || \
      pip install --quiet fastapi uvicorn httpx pydantic-settings \
                          "python-jose[cryptography]" sqlalchemy aiosqlite \
        || warn "pip install failed — context-api may not start"
  else
    warn "pip not found — context-api will only work if its deps are already installed"
  fi
  ...
  boot pseudo-iam       "cd pseudo-iam-service        && JWT_SECRET=\"$JWT_SECRET\" PORT=8101 npm run dev"
  boot audit-gov        "cd audit-governance-service  && DATABASE_URL=\"$DATABASE_URL_AUDIT_GOV\" PORT=8500 npm run dev"
  sleep 2

  boot agent-service    "cd agent-and-tools/apps/agent-service   && PORT=3001 AUDIT_GOV_URL=\"$AUDIT_GOV_URL\" JWT_SECRET=\"$JWT_SECRET\" npm run dev"
  boot tool-service     "cd agent-and-tools/apps/tool-service    && PORT=3002 DATABASE_URL=\"$DATABASE_URL_AGENT_TOOLS\" AUDIT_GOV_URL=\"$AUDIT_GOV_URL\" JWT_SECRET=\"$JWT_SECRET\" npm run dev"
  boot agent-runtime    "cd agent-and-tools/apps/agent-runtime   && PORT=3003 DATABASE_URL=\"$DATABASE_URL_AGENT_TOOLS\" AUDIT_GOV_URL=\"$AUDIT_GOV_URL\" JWT_SECRET=\"$JWT_SECRET\" npm run dev"
  ...
  boot context-api      "cd context-fabric/services/context_api_service && DATABASE_URL=\"$DATABASE_URL_AUDIT_GOV\" PORT=8000 AUDIT_GOV_URL=\"$AUDIT_GOV_URL\" MCP_SERVER_URL=\"$MCP_SERVER_URL\" MCP_BEARER_TOKEN=\"$MCP_BEARER_TOKEN\" python3 -m uvicorn app.main:app --host 0.0.0.0 --port 8000"
  sleep 3
...
cmd_down() {
  ...
  for p in 3000 3001 3002 3003 3004 5174 7100 8000 8080 8101 8500; do
