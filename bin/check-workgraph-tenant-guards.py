...
ROUTE_TENANT_POLICIES: dict[str, str] = {
    ...
    "/api/authz workflowAuthzRouter": "iam_control_plane: effective access is resolved by IAM with an explicit tenant subject",
    "/api/workflow-templates workflowTemplatesRouter": "capability_scoped_authz: workflow templates carry tenant ownership and central resource/action access decisions",
    "/api/workflows workflowTemplatesRouter": "capability_scoped_authz: compatibility alias for tenant-owned workflow templates",
    ...
    "/api/notifications notificationsRouter": "tenant_scoped_runtime: user notifications are tenant-scoped activity and delivery records",
    ...
    "/api/runtime-policy runtimePolicyRouter": "tenant_scoped_runtime: runtime devices, policies, and consent are tenant-scoped",
    "/api/planning/capacity capacityRouter": "capability_scoped_authz: capacity planning is authenticated and capability/team scoped",
    "/api/verifications verificationRouter": "tenant_scoped_runtime: verification records are linked to tenant workflow evidence",
    "/api/work-programs workProgramsRouter": "tenant_scoped_runtime: work-program runs and items are tenant-scoped",
    "/api/workflow-debug workflowDebugRouter": "tenant_scoped_runtime: debug and simulation access is tenant-scoped and authenticated",
    "/api/workflow-instances workflowLifecycleRouter": "tenant_scoped_runtime: lifecycle mutations operate on tenant-scoped workflow instances",
    "/api/workflows workflowLifecycleRouter": "tenant_scoped_runtime: compatibility lifecycle routes operate on tenant-scoped workflow instances",
    "/api/workflow-operations workflowOperationsRouter": "tenant_scoped_runtime: operations aggregates and recovery actions are tenant-scoped with explicit IAM permissions",
    "/api/loop-strategies loopStrategyRouter": "tenant_scoped_runtime: reusable loop strategies are tenant-scoped configuration",
