...
#
#   bin/box.sh up           start the core box (no image builds — fast)
#   bin/box.sh up --build   start the core box, rebuilding changed images first
#   bin/box.sh up --with-verifier --with-compression --with-legacy-ui
#   bin/box.sh rebuild      rebuild the feature images (context-api, workgraph-api,
#                           platform-web) after a git pull
...
banner() {
  ...
  echo "  Legacy split UIs are available only with the frontend-legacy profile."
  echo "════════════════════════════════════════════════════════════════════"
...
case "${1:-}" in
  ...
  status)
    ...
    printf '  platform-web :5180 … '; curl -fsS -o /dev/null http://localhost:5180/ && echo OK || echo DOWN
  ...
  logs)
    ...
    ;;
  down)
    dc stop $CORE_APPS formal-verifier prompt-compressor workgraph-web blueprint-workbench user-and-capability portal edge-gateway $BOOTSTRAP $INFRA
    echo "[box] stopped (data kept; 'docker compose down -v' to wipe)."
    ;;
  *)
    echo "usage: $0 {up [--build] [--with-verifier] [--with-compression] [--with-legacy-ui]|rebuild|seed|status|logs <svc>|down}" >&2
