...
load_env() {
  ...
  WORKGRAPH_DB_TENANT_ISOLATION_REQUIRED="${WORKGRAPH_DB_TENANT_ISOLATION_REQUIRED:-false}"
...
start_apps() {
  ...
  docker run -d \
    ...
    -e ENVIRONMENT="$ENVIRONMENT" \
    -e SINGULARITY_ENV="$SINGULARITY_ENV" \
    -e AUTH_PROVIDER="${AUTH_PROVIDER:-iam}" \
    ...
    -e IAM_SERVICE_TOKEN_TENANT_IDS="${IAM_SERVICE_TOKEN_TENANT_IDS:-default}" \
  ...
  docker run -d \
    ...
    -e REQUIRE_TENANT_ID="$REQUIRE_TENANT_ID" \
    -e WORKGRAPH_DB_TENANT_ISOLATION_REQUIRED="$WORKGRAPH_DB_TENANT_ISOLATION_REQUIRED" \
    -e DEFAULT_GOVERNANCE_MODE="$DEFAULT_GOVERNANCE_MODE" \
