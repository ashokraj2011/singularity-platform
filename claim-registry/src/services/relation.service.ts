...
export async function assertRelation(input: AssertRelationInput) {
  ...
  const [from, to] = await Promise.all([
    prisma.claim.findFirst({ where: { id: input.fromClaimId, tenantId }, select: { id: true } }),
    prisma.claim.findFirst({ where: { id: input.toClaimId, tenantId }, select: { id: true } }),
  ]);
  ...
  const relation = await prisma.claimRelation.upsert({
    ...
    create: { tenantId, fromClaimId: input.fromClaimId, toClaimId: input.toClaimId, type: input.type as never, note: input.note ?? null, createdBy: input.createdBy },
  });
...
export async function listRelations(claimId: string) {
  return prisma.claimRelation.findMany({
    where: { tenantId: currentRegistryTenant(), OR: [{ fromClaimId: claimId }, { toClaimId: claimId }] },
