...
### Optional — local gateway for the blue Workbench cockpit

Bare-metal serves the native console at `:5180/workbench`. To run the real `blueprint-workbench` cockpit under one origin (so a CALL_WORKFLOW "Open Workbench" launch and shared auth work), run `bin/local-gateway.sh up` (needs Docker, no host nginx). Enter at `http://localhost:8085` and view runs there. Per the `:5176`/`:8085` sweep noted above, start the gateway after the stack is up. See [`edge-gateway/README.md`](edge-gateway/README.md).
